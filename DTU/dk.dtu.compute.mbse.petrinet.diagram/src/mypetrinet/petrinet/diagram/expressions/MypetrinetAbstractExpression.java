 ...
 * @generated
 */
public abstract class MypetrinetAbstractExpression {
	...
	*/
	protected void setStatus(int severity, String message, Throwable throwable) {
		String pluginID = MypetrinetDiagramEditorPlugin.ID;
		this.status = new Status(severity, pluginID, -1, (message != null) ? message : "", throwable); //$NON-NLS-1$
		if (!this.status.isOK()) {
			MypetrinetDiagramEditorPlugin.getInstance().logError("Expression problem:" + message + "body:" + body(), //$NON-NLS-1$//$NON-NLS-2$
					throwable);
		}
	}
	...
	* @generated
	*/
	protected MypetrinetAbstractExpression(String body, EClassifier context) {
		myBody = body;
		myContext = context;
	}
	...
	public Object evaluate(Object context, Map env) {
		if (context().isInstance(context)) {
			try {
				...
				MypetrinetDiagramEditorPlugin.getInstance().logError("Expression evaluation failure: " + body(), e); //$NON-NLS-1$
			}
		}
		return null;
	}
